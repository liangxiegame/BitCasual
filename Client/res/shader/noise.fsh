...
//        vec2 displacedTexCoord = v_texCoord + vec2(texture2D(u_noise, v_texCoord +  vec2(sin(u_interpolate) * 0.1 ,0)).x,texture2D(u_noise, v_texCoord + vec2(0,cos(u_interpolate) * 0.1)).y) * 0.25;
    ...
    vec2 displacedTexCoord = v_texCoord + (vec2(texture2D(u_noise,v_texCoord + vec2(cos(u_interpolate) * 0.1,sin(u_interpolate) * 0.1))) - vec2(0.5,0.5)) * 0.01;
//    vec2 displacedTexCoord = v_texCoord + (vec2(texture2D(u_noise,v_texCoord + vec2(sin(u_interpolate) * 0.1, cos(u_interpolate) * 0.1))) - vec2(0.5,0.5)) * 0.01;
//                                               texture2D(u_noise,v_texCoord + vec2(0,cos(u_interpolate) * 0.1)).z - 0.5) * 0.01;
    
    vec4 color1 = texture2D(CC_Texture0, v_texCoord);
    vec4 color2 = texture2D(CC_Texture0, displacedTexCoord);
//    vec4 color2 = texture2D(u_noise,v_texCoord);
